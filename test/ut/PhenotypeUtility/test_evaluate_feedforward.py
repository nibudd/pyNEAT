...
def test_multiple_inputs_sum_weighted_contributions():
    ...
    edge1 = EdgeGene(
        id=0, innovation_id=0, in_node=input1, out_node=output, weight=0.5, enabled=True
    )
    edge2 = EdgeGene(
        id=1,
        innovation_id=1,
...
def test_two_hidden_layers_propagate_sequentially():
    ...
    genotype = Genotype(
        [input_node, hidden1, hidden2, output_node], [edge1, edge2, edge3]
    )
    inputs = np.array([1.0])
    result = evaluate_feedforward(genotype, inputs, sigmoid)
...
def test_scrambled_node_order_still_evaluates_correctly():
    ...
    genotype = Genotype(
        [output_node, hidden2, input_node, hidden1], [edge1, edge2, edge3]
    )
    inputs = np.array([1.0])
    result = evaluate_feedforward(genotype, inputs, sigmoid)
...
def test_diamond_topology_merges_parallel_paths():
    ...
    genotype = Genotype(
        [input_node, hidden1, hidden2, output_node], [edge1, edge2, edge3, edge4]
    )
    inputs = np.array([1.0])
    result = evaluate_feedforward(genotype, inputs, sigmoid)
